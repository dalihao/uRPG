...
class SpaceAlloc:
	...
	def __init__(self, utype):
		...
		# 副本如果没有创建有哪些人在其中登录，   一旦创建完毕会将他们创建到副本的cellapp上
		self._pendingEnterEntityMBs = {}
		# 副本如果没有创建有哪些人请求进入，   一旦创建完毕会将他们扔到副本
	...
	def createSpace(self, spaceKey, context):
		...
		DEBUG_MSG("创建固定空间---------spaceUType= %i-------spacekey= %i--------" % (self._utype, spaceKey))

		context = copy.copy(context)

		spaceData = d_spaces.datas.get(self._utype)
		DEBUG_MSG("创建空间名字spacekey= %s  -------type %s" % (spaceData["name"], spaceData["entityType"]))
...
class SpaceAllocDuplicate_nouse(SpaceAlloc):
	"""
	副本分配器
	"""
	def __init__(self, utype):
