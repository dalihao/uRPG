...
class State:
	...
	def onStateChanged_(self, oldstate, newstate):
		...
		#INFO_MSG("%s:onStateChanged_: %i oldstate=%i to newstate=%i, forbids=%s, subState=%i." % (self.getScriptName(),self.id, oldstate, newstate, self._forbidCounter, self.subState))
	...
	def changeState(self, state):
		...
		if state == GlobalDefine.ENTITY_STATE_REST:
			if self.mabitimeid > 0:
				self.delTimer(self.mabitimeid)
			self.mabitimeid = self.addTimer(3, 0, SCDefine.TIMER_TYPE_MABI)
