...
class AI:
	...
	def checkInTerritory(self):
		...
		ret = abs(self.spawnPos.x - self.position.x) <= __TERRITORY_AREA__
	...
	def enable(self):
		...
		# 心跳timer, 每1秒一次
		self.enablecount += 1
		if self.enablecount == 1:
			self.heartBeatTimerID =	self.addTimer(random.randint(0, 1),self.attspeed / 100, SCDefine.TIMER_TYPE_HEARDBEAT)
	...
	def onThinkFight(self):
		...
		if abs(entity.position.x - self.position.x) > attackMaxDist:
			runSpeed = self.getDatas()["runSpeed"]
			if runSpeed != self.moveSpeed:
				self.moveSpeed = runSpeed
			self.gotoPosition(entity.position, attackMaxDist - 0.1)
			return
		else:
			self.resetSpeed()
	...
	def onStateChanged_(self, oldstate, newstate):
		...
		entity状态改变了
		"""
		if self.isState(GlobalDefine.ENTITY_STATE_DEAD) or self.isState(GlobalDefine.ENTITY_STATE_REST):
			if self.isMoving:
				self.stopMotion()
	...
	def onEnterTrap(self, entityEntering, range_xz, range_y, controllerID, userarg):
		...
		#DEBUG_MSG("%s::onEnterTrap: %i entityEntering=(%s)%i, range_xz=%s, range_y=%s, controllerID=%i, userarg=%i" %(self.getScriptName(), self.id, entityEntering.getScriptName(), entityEntering.id, range_xz, range_y, controllerID, userarg))
		if entityEntering.getCurrSpace().name == "BOSS之家" or entityEntering.getCurrSpace().name == "神庙" or "地狱塔" in entityEntering.getCurrSpace().name:
			return
		else:
	...
	def onLeaveTrap(self, entityLeaving, range_xz, range_y, controllerID, userarg):
		...
		#INFO_MSG("%s::onLeaveTrap: %i entityLeaving=(%s)%i." % (self.getScriptName(), self.id,entityLeaving.getScriptName(), entityLeaving.id))
	...
	def onLoseTarget(self):
		...
		敌人丢失
		"""
		#INFO_MSG("%s::onLoseTarget: %i target=%i, enemyLogSize=%i." % (self.getScriptName(), self.id,self.targetID, len(self.enemyLog)))
